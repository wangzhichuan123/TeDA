...
if __name__ == "__main__":
    data_dir = "data/OS-ESB-core"
    train_dataset = ESBCoreDataset(data_dir, "train")
    print(len(train_dataset))
    for i in range(len(train_dataset)):
        print(train_dataset[i][1])
        break
    query_dataset = ESBCoreDataset(data_dir, "query")
    target_dataset = ESBCoreDataset(data_dir, "target")
    seen_categories = train_dataset.label_list
    import pdb

    pdb.set_trace()
    train_dataset = ESBCoreDataset(data_dir, "train", modality="point")
    pc, label = train_dataset[0]
    train_dataset = ESBCoreDataset(data_dir, "train", modality="vox")
