...
def eval_all_metric(query_fts, target_fts, query_lbls, target_lbls, metric="cosine"):
    ...
    query_fts = query_fts.detach().cpu().numpy()
    target_fts = target_fts.detach().cpu().numpy()
    ...
    print(f"{s_map:10.4f}|{s_ndcg:10.4f}|{s_anmrr:10.4f}|{s_recall:10.4f}")
    print(f"pr curve: \n{s_pr}")
...
def map_score_sim(dist_mat, lbl_a, lbl_b, top_k=1e9):
    # print(dist_mat.shape)
    dist_mat = dist_mat.detach().cpu().numpy()
    n_a, n_b = dist_mat.shape
    top_k = min(top_k, n_b)
    s_idx = dist_mat.argsort(axis=1)[:, ::-1]
    res = []
    for i in range(n_a):
        order = s_idx[i]
        p_list = []
